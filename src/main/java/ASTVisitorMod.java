...
public class ASTVisitorMod extends ASTVisitor {
    ...
    public boolean visit(InfixExpression node) {
        if (!this.oprt.containsKey(node.getOperator().toString())) {
            ...
            this.oprt.put(node.getOperator().toString(),
						  this.oprt.get(node.getOperator().toString()) + 1);
        }
        return true;
    }
    ...
    public boolean visit(SimpleName node) {
        if (!this.names.containsKey(node.getIdentifier())) {
            ...
            this.names.put(node.getIdentifier(),
						   this.names.get(node.getIdentifier()) + 1);
        }
        return true;
    }
    ...
    public boolean visit(StringLiteral node) {
        ...
        if (!this.names.containsKey(node.getLiteralValue())) {
            ...
            this.names.put(node.getLiteralValue(),
						   this.names.get(node.getLiteralValue()) + 1);
        }
        return true;
    }
    ...
    public boolean visit(CharacterLiteral node) {
        if (!this.names.containsKey(Character.toString(node.charValue()))) {
            ...
            this.names.put(Character.toString(node.charValue()),
						   this.names.get(Character.toString(node.charValue()))
						   + 1);
        }
        return true;
    }
    ...
    public boolean visit(BooleanLiteral node) {
        if (!this.names.containsKey(Boolean.toString(node.booleanValue()))) {
            ...
            this.names.put(Boolean.toString(node.booleanValue()),
						   this.names.get(Boolean.toString(node.booleanValue()))
						   + 1);
        }
        return true;
    }
    ...
    public boolean visit(NumberLiteral node) {
        if (!this.names.containsKey(node.getToken())) {
            ...
            this.names.put(node.getToken(),
                           this.names.get(node.getToken()) + 1);
        }
